...
class WC_Optima_API
{
    ...
    /**
     * Get invoice PDF from Optima API
     *
     * @param string $invoice_id Invoice ID
     * @return string|false PDF content as string or false on failure
     */
    public function get_invoice_pdf_by_id($invoice_id)
    {
        $token = $this->get_access_token();
        ...
        try {
            ...
            if (!class_exists('\GuzzleHttp\Client')) {
                // Since Guzzle isn't available, use WordPress HTTP API
                return $this->get_invoice_pdf_with_wp_http($token, $invoice_id);
            }
            ...
            $response = $client->request('GET', $this->api_url . '/Invoices/' . $invoice_id . '/pdf', $options);
            $pdf_content = $response->getBody()->getContents();

            return $pdf_content;
        } catch (Exception $e) {
            error_log(__('Integracja WC Optima: Błąd podczas pobierania PDF faktury - ', 'optima-woocommerce') . $e->getMessage());
            // Fall back to WordPress HTTP API
            return $this->get_invoice_pdf_with_wp_http($token, $invoice_id);
        }
    ...
    /**
     * Get invoice PDF using WordPress HTTP API as a fallback
     *
     * @param string $token The access token
     * @param string $invoice_id Invoice ID
     * @return string|false PDF content as string or false on failure
     */
    private function get_invoice_pdf_with_wp_http($token, $invoice_id)
    {
        $response = wp_remote_get($this->api_url . '/Invoices/' . $invoice_id . '/pdf', [
            'timeout' => 60,
            'redirection' => 5,
            'httpversion' => '1.0',
            'headers' => [
        ...
        $status_code = wp_remote_retrieve_response_code($response);
        if ($status_code !== 200) {
            error_log(sprintf(__('Integracja WC Optima - Błąd API (%d) podczas pobierania PDF faktury', 'optima-woocommerce'), $status_code));
            return false;
        }
